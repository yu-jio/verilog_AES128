#define _CRT_SECURE_NO_WARNINGS
#include <stdio.h>

typedef unsigned char byte;
typedef unsigned int word;

#define Nb 4
#define Nk 4
#define Nr 10
#define HIHEX(x) (x>>4)
#define LOWHEX(x) (x&0x0F)

byte SBOX[256] = {
	
};
byte InvSBOX[256] = {
		,
};
word Rcon[11] = {
	0x00000000,0x01000000, 0x02000000, 0x04000000, 0x08000000,0x10000000,0x20000000,0x40000000,0x80000000,0x1B000000,0x36000000
};

void Cipher(byte* in, byte* out, word* w);
void InvCipher(byte* in, byte* out, word* w);
void KeyExpansion(byte* key, word* w);

int main()
{
	byte textin[4 * Nb];
	byte key[4 * Nk];
	byte out[4 * Nb];
	word w[Nb * (Nr + 1)];

	int ENCDEC;

	printf("[Encrytion -> 1, Decryption -> 0] : ");
	scanf("%d", &ENCDEC);
	if (ENCDEC == 1)
	{
		printf("Key(128) : ");
		for (int i = 0; i < Nk * 4; i++)
		{
			scanf("%02hhx", &key[i]);
		}
		printf("Plaintext : ");
		for (int i = 0; i < Nb * 4; i++)
		{
			scanf("%02hhx", &textin[i]);
		}
		KeyExpansion(key, w);
		Cipher(textin, out, w);
		printf("Ciphertext : ");
		for (int i = 0; i < 16; i++)
		{
			printf("%02hhx", out[i]);
		}
	}
	else if (ENCDEC == 0)
	{
		printf("Key(128) : ");
		for (int i = 0; i < Nk * 4; i++)
		{
			scanf("%02hhx", &key[i]);
		}
		printf("Ciphertext : ");
		for (int i = 0; i < Nb * 4; i++)
		{
			scanf("%02hhx", &textin[i]);
		}
		KeyExpansion(key, w);
		InvCipher(textin, out, w);
		printf("Plaintext : ");
		for (int i = 0; i < 16; i++)
		{
			printf("%02hhx", out[i]);
		}
	}
}
word RotWord(word w)
{
	w = (w >> 24) | (w << 8);
	return w;
}


word SubWord(word w)
{
	int idx = 0;
	for (int i = 0; i < 4; i++)
	{
		w = ((SBOX[(w >> (24 - (8 * i))) & 0xFF]) << (24 - (8 * i))) | w & ~(0xFF << (24 - (8 * i)));
	}
	return w;
}

byte Mul(byte state)
{
	return (state << 1) ^ (((state >> 7) & 0x01) * 0x1b);
}

void SubBytes(byte state[4 * Nb])
{
	for (int i = 0; i < Nb; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			state[4 * i + j] = SBOX[state[4 * i + j]];
		}
	}
}
void ShiftRows(byte* state)
{
	word temp;

	int i;

	for (i = 1; i < 4; i++)
	{
		temp = (state[i] << 24) | (state[i + 4] << 16) | (state[i + 8] << 8) | (state[i + 12]);
		temp = (temp >> (32 - (i * 8))) | (temp << (i * 8));
		state[i] = (temp >> 24) & 0xFF;
		state[i + 4] = (temp >> 16) & 0xFF;
		state[i + 8] = (temp >> 8) & 0xFF;
		state[i + 12] = (temp) & 0xFF;
	}
}
void MixColumns(byte* state)
{
	int i;
	byte temp[16];
	for (i = 0; i < 4; i++)
	{
		temp[(i * 4) + 0] = Mul(state[(i * 4) + 0]) ^
			(Mul(state[(i * 4) + 1]) ^ state[(i * 4) + 1]) ^
			state[(i * 4) + 2] ^
			state[(i * 4) + 3];

		temp[(i * 4) + 1] = state[(i * 4) + 0] ^
			Mul(state[(i * 4) + 1]) ^
			(Mul(state[(i * 4) + 2]) ^ state[(i * 4) + 2]) ^
			state[(i * 4) + 3];

		temp[(i * 4) + 2] = state[(i * 4) + 0] ^
			state[(i * 4) + 1] ^
			Mul(state[(i * 4) + 2]) ^
			(Mul(state[(i * 4) + 3]) ^ state[(i * 4) + 3]);

		temp[(i * 4) + 3] = (Mul(state[(i * 4) + 0]) ^ state[(i * 4) + 0]) ^
			state[(i * 4) + 1] ^
			state[(i * 4) + 2] ^
			Mul(state[(i * 4) + 3]);
	}
	memcpy(state, temp, 16);
}

void AddRoundKey(byte* state, const word* w)
{
	int i, j;

	for (i = 0; i < Nb; i++)
	{
		for (j = 0; j < Nb; j++)
		{
			state[(i * 4) + j] ^= (w[i] >> (24 - (8 * j))) & 0xFF;
		}
	}

}

void KeyExpansion(byte* key, word* w)
{
	int i = 0;
	word temp;

	while (i < Nk)
	{
		w[i] = ((key[4 * i] << 24) | (key[4 * i + 1] << 16) | (key[4 * i + 2] << 8) | key[4 * i + 3]);
		i = i + 1;
	}

	i = Nk;
	while (i < Nb * (Nr + 1))
	{
		temp = w[i - 1];

		if ((i % Nk) == 0)
		{
			temp = SubWord(RotWord(temp)) ^ Rcon[i / Nk];
		}
		w[i] = w[i - Nk] ^ temp;
		i = i + 1;
	}
}

void InvSubBytes(byte state[4 * Nb])
{
	for (int i = 0; i < Nb; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			state[4 * i + j] = InvSBOX[state[4 * i + j]];
		}
	}
}
void InvShiftRows(byte* state)
{
	word temp;

	int i;

	for (i = 1; i < 4; i++)
	{
		temp = (state[i] << 24) | (state[i + 4] << 16) | (state[i + 8] << 8) | (state[i + 12]);
		temp = (temp >> (32 - ((4 - i) * 8))) | (temp << ((4 - i) * 8));
		state[i] = (temp >> 24) & 0xFF;
		state[i + 4] = (temp >> 16) & 0xFF;
		state[i + 8] = (temp >> 8) & 0xFF;
		state[i + 12] = (temp) & 0xFF;
	}
}
void InvMixColumns(byte* state)
{
	int i;
	byte temp[16];
	for (i = 0; i < 4; i++)
	{
		temp[(i * 4) + 0] =
			(Mul(Mul(Mul(state[(i * 4) + 0]) ^ state[(i * 4) + 0]) ^ state[(i * 4) + 0])) ^
			(Mul(Mul(Mul(state[(i * 4) + 1])) ^ state[(i * 4) + 1]) ^ state[(i * 4) + 1]) ^
			(Mul(Mul(Mul(state[(i * 4) + 2]) ^ state[(i * 4) + 2])) ^ state[(i * 4) + 2]) ^
			(Mul(Mul(Mul(state[(i * 4) + 3]))) ^ state[(i * 4) + 3]);

		temp[(i * 4) + 1] =
			(Mul(Mul(Mul(state[(i * 4) + 0]))) ^ state[(i * 4) + 0]) ^
			(Mul(Mul(Mul(state[(i * 4) + 1]) ^ state[(i * 4) + 1]) ^ state[(i * 4) + 1])) ^
			(Mul(Mul(Mul(state[(i * 4) + 2])) ^ state[(i * 4) + 2]) ^ state[(i * 4) + 2]) ^
			(Mul(Mul(Mul(state[(i * 4) + 3]) ^ state[(i * 4) + 3])) ^ state[(i * 4) + 3]);

		temp[(i * 4) + 2] =
			(Mul(Mul(Mul(state[(i * 4) + 0]) ^ state[(i * 4) + 0])) ^ state[(i * 4) + 0]) ^
			(Mul(Mul(Mul(state[(i * 4) + 1]))) ^ state[(i * 4) + 1]) ^
			(Mul(Mul(Mul(state[(i * 4) + 2]) ^ state[(i * 4) + 2]) ^ state[(i * 4) + 2])) ^
			(Mul(Mul(Mul(state[(i * 4) + 3])) ^ state[(i * 4) + 3]) ^ state[(i * 4) + 3]);

		temp[(i * 4) + 3] =
			(Mul(Mul(Mul(state[(i * 4) + 0])) ^ state[(i * 4) + 0]) ^ state[(i * 4) + 0]) ^
			(Mul(Mul(Mul(state[(i * 4) + 1]) ^ state[(i * 4) + 1])) ^ state[(i * 4) + 1]) ^
			(Mul(Mul(Mul(state[(i * 4) + 2]))) ^ state[(i * 4) + 2]) ^
			(Mul(Mul(Mul(state[(i * 4) + 3]) ^ state[(i * 4) + 3]) ^ state[(i * 4) + 3]));
	}
	memcpy(state, temp, 16);
}

void Cipher(byte* in, byte* out, word* w)
{
	byte state[4 * Nb] = { 0x00, };
	int round = 0;

	memcpy(state, in, 16);

	AddRoundKey(state, &w[round * Nb]);

	for (round = 1; round < Nr; round++)
	{
		SubBytes(state);
		ShiftRows(state);
		MixColumns(state);
		AddRoundKey(state, &w[round * Nb]);
	}

	SubBytes(state);
	ShiftRows(state);
	AddRoundKey(state, &w[Nr * Nb]);

	memcpy(out, state, 16);
}

void InvCipher(byte* in, byte* out, word* w)
{
	byte state[4 * Nb] = { 0x00, };
	int round = 10;

	memcpy(state, in, 16);

	AddRoundKey(state, &w[round * Nb]);

	for (round = Nr - 1; round > 0; round--)
	{
		InvShiftRows(state);
		InvSubBytes(state);
		AddRoundKey(state, &w[round * Nb]);
		InvMixColumns(state);
	}

	InvShiftRows(state);
	InvSubBytes(state);
	AddRoundKey(state, &w[0]);

	memcpy(out, state, 16);
}

